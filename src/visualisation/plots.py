...
def visualisation_1_1(lab_data: pd.DataFrame):
    ...
    us_depth_fig = create_flow_us_depth_plot(
        simple_combined_model.df,
        setups,
        title="Simple Combined Model Performance for 3 Barrier Setups",
    )
    for setup in setups:
        min_us_depth = (
            simple_combined_model.df[
                simple_combined_model.df["Barrier Setup"] == setup
            ]["Mean Upstream Depth (mm)"].min()
            / 1000
        )
        max_us_depth = (
            simple_combined_model.df[
                simple_combined_model.df["Barrier Setup"] == setup
            ]["Mean Upstream Depth (mm)"].max()
            / 1000
        )
        add_function_to_plot(
            us_depth_fig,
            simple_combined_model.plotting_function,
            (min_us_depth, max_us_depth),
            0.001,
            f"{setup} Model",
            1000,
            1,
...
def visualisation_1_2(lab_data: pd.DataFrame):
    ...
    us_depth_fig = create_flow_us_depth_plot(
        simple_combined_model.df,
        setups,
        title="Simple IF Model Performance for 3 Barrier Setups",
    )
    for setup in setups:
        min_us_depth = (
            simple_combined_model.df[
...
def visualisation_1_3(lab_data: pd.DataFrame):
    ...
    for setup in tqdm(setups):
        us_depth_fig = create_flow_us_depth_plot(
            simple_combined_model.df,
            [setup],
            title=f"Simple Combined Model Performance for {setup}",
        )
        min_us_depth = (
            simple_combined_model.df[
                simple_combined_model.df["Barrier Setup"] == setup
...
def visualisation_1_5(lab_data: pd.DataFrame):
    ...
    for setup in tqdm(setups):
        us_depth_fig = create_flow_us_depth_plot(
            simple_combined_model.df,
            [setup],
            title=f"Advanced Combined Model Performance for {setup}",
        )
        min_us_depth = (
            simple_combined_model.df[
                simple_combined_model.df["Barrier Setup"] == setup
...
def visualisation_1_6(lab_data: pd.DataFrame):
    ...
    for file_path in tqdm(file_paths):
        ...
        fig = create_barrier_depth_diagram(
            barrier_setup, us_profile, ds_profile, point_data, title
        )
...
def visualisation_1_7(measured_friction_data: pd.DataFrame):
    ...
    for file_path in tqdm(file_paths):
        ...
        fig = create_friction_depth_diagram(
            incline_pct, x_profile, depth_profile, point_data, title
        )
        fig.savefig(output_directory / f"{file_path.stem}.svg")
        plt.close(fig)
