...
def reproduce_friction_experiments():
    ...
    for (flow_ls, incline_pct), group in grouped:
        ...
        try:
            flume = Flume(
                barrier_setup=None, set_flow=flow_m3s, incline=incline_fraction
            )
            ...
            print(
                f"Solver failed to run for Flow: {flow_ls}, Incline: {incline_pct}. Error: {e}"
            )
...
def reproduce_barrier_experiments(df: pd.DataFrame):
    ...
    for (barrier_setup, flow_ls), group in grouped:
        ...
        try:
            flume = Flume(
                barrier_setup=barrier_setup, set_flow=flow_m3s, incline=incline_fraction
            )
            profile = flume.simulate()
            ...
            print(
                f"Solver failed to run for Barrier Setup: {barrier_setup}, Flow: {flow_ls}. Error: {e}"
            )
