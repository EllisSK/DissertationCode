...
def make_lab_data(operation_mode: str, barrier_setup: str, flows_depths: list[tuple[float, float]]) -> pd.DataFrame:
...
def synthetic_sluice_data(coeff: float, gap_mm: int, depths_mm: list[float]) -> pd.DataFrame:
...
class TestSimpleSluiceModel:
    def test_equation(self):
        model = SimpleSluiceModel("test", make_lab_data("Sluice", "100-100-50", [(10.0, 250.0)]))
        # Q = C * a * sqrt(h)
        assert model._equation((0.25, 0.1), 2.0) == pytest.approx(2.0 * 0.1 * np.sqrt(0.25))

    def test_fit_recovers_known_coefficient(self):
        model = SimpleSluiceModel("test", synthetic_sluice_data(2.1, 100, [150.0, 250.0, 350.0, 450.0]))
        model.fit()
        assert model.optimal == pytest.approx(2.1, rel=1e-6)

    def test_predict_before_fit_raises(self):
        model = SimpleSluiceModel("test", synthetic_sluice_data(2.1, 100, [150.0, 250.0]))
        with pytest.raises(Exception, match="hasn't been fit"):
            model.predict((0.25, 0.1))

    def test_perfect_fit_metrics(self):
        model = SimpleSluiceModel("test", synthetic_sluice_data(2.1, 100, [150.0, 250.0, 350.0]))
        model.fit()
        rmse, mae, bias, var, corr, kge, r2 = model._calculate_objective_functions(model.df)
...
class TestSimpleOrificeModel:
    def test_equation(self):
        model = SimpleOrificeModel("test", make_lab_data("Orifice", "0-100-0", [(10.0, 350.0)]))
        # Q = C * (h_bottom^1.5 - h_top^1.5)
        assert model._equation((0.15, 0.05), 2.0) == pytest.approx(2.0 * (0.15**1.5 - 0.05**1.5))
    ...
    def test_orifice_geometry_top_gap(self):
        ...
        model = SimpleOrificeModel("test", make_lab_data("Orifice", "0-0-50", [(10.0, 400.0)]))
        row = model.df.iloc[0]
...
class TestAdvancedModels:
    def test_sluice_equation_analytic(self):
        model = AdvancedSluiceModel("test", make_lab_data("Sluice", "100-100-50", [(10.0, 250.0)]))
        cc = np.pi / (np.pi + 2)
        cd = cc * 0.98
        head = 0.25 - cc * 0.1
        assert model._equation((0.25, 0.1)) == pytest.approx(cd * 0.1 * np.sqrt(2 * 9.80665 * head))

    def test_weir_equation_analytic(self):
        model = AdvancedWeirModel("test", make_lab_data("Weir", "0-0-0", [(10.0, 450.0)]))
        cd = (np.pi / (np.pi + 2)) * 0.98
        assert model._equation((0.05, 0.4)) == pytest.approx((2 / 3) * cd * np.sqrt(2 * 9.80665) * 0.05**1.5)

    def test_orifice_equation_analytic(self):
        model = AdvancedOrificeModel("test", make_lab_data("Orifice", "0-100-0", [(10.0, 350.0)]))
        cd = (np.pi / (np.pi + 2)) * 0.98
...
def combined_lab_data() -> pd.DataFrame:
    ...
    return pd.concat(
        [
            make_lab_data("Sluice", "100-100-50", [(8.0, 150.0), (10.0, 250.0), (12.0, 300.0)]),
            make_lab_data("Weir", "0-0-0", [(6.0, 420.0), (10.0, 450.0), (14.0, 480.0)]),
            make_lab_data("Orifice", "0-100-0", [(7.0, 320.0), (9.0, 350.0), (11.0, 380.0)]),
        ],
        ignore_index=True,
    )
